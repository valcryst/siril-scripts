    ...
    try:
        with fits.open(file) as hdul:
            ...
            if None in (temp, exptime, naxis1, naxis2, xbin, ybin) or (iso is None and gain is None):
                return None
            return {
                "path": file, "ccd_temp": temp, "iso": iso, "gain": gain,
                "exptime": exptime, "naxis1": naxis1, "naxis2": naxis2,
...
class DarkLibraryApp:
    def __init__(self, root):
        self.root = root
        self.root.title("FITS Dark-O-Mat - 2025.06-1")
    ...
    def rescan_library(self):
        ...
        with sqlite3.connect(db_path) as conn:
            ...
            for f in fits_files:
                ...
                if entry:
                    c.execute("""
                        INSERT INTO darks (path, ccd_temp, iso, gain, exptime, naxis1, naxis2, xbinning, ybinning)
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                        (entry["path"], entry["ccd_temp"], entry["iso"], entry["gain"],
                         entry["exptime"], entry["naxis1"], entry["naxis2"],
                         entry["xbinning"], entry["ybinning"]))
            conn.commit()
    ...
    def create_widgets(self):
        ...
        self.temp_var, self.iso_var, self.exptime_var = tk.StringVar(), tk.StringVar(), tk.StringVar()
        self.res_var, self.bin_var = tk.StringVar(), tk.StringVar()

        labels_and_vars = [
            ("Temperature", self.temp_var, "temp_cb"),
        ...
        for idx, (label, var, cb_name) in enumerate(labels_and_vars):
        ...
        ttk.Label(self.criteria_frame, text="Matching darks:").grid(row=5, column=0, sticky="w")
        self.matching_count = tk.StringVar(value="0")
        ttk.Label(self.criteria_frame, textvariable=self.matching_count).grid(row=5, column=1, sticky="w")

        ttk.Label(self.criteria_frame, text="Number of darks to use:").grid(row=6, column=0, sticky="w")
        self.slider_value = tk.StringVar(value="2")
        ttk.Label(self.criteria_frame, textvariable=self.slider_value).grid(row=6, column=1, sticky="w")

        self.slider = ttk.Scale(self.criteria_frame, from_=2, to=2, orient="horizontal", command=self.update_slider_label)
        self.slider.grid(row=7, column=0, columnspan=3, sticky="ew", pady=(2, 10))

        ttk.Button(self.criteria_frame, text="Select target directory", command=self.select_target_dir).grid(row=8, column=0, pady=(10, 0), sticky="w")
        self.target_dir = tk.StringVar()
        ttk.Label(self.criteria_frame, textvariable=self.target_dir).grid(row=8, column=1, padx=10, sticky="ew")
        self.create_btn = ttk.Button(self.criteria_frame, text="Create Master Dark", command=self.create_master_dark, state="disabled")
        self.create_btn.grid(row=9, column=0, columnspan=2, pady=10)
    ...
    def populate_criteria(self):
        ...
        with sqlite3.connect(db) as conn:
            ...
            for col, var, cb_name in [
                ("ccd_temp", self.temp_var, "temp_cb"),
    ...
    def check_all_selected(self):
        if all([self.temp_var.get(), self.iso_var.get(), self.exptime_var.get(), self.res_var.get(), self.bin_var.get()]):
    ...
    def update_matching_files(self):
        ...
        where = []
        params = []
        for col, var in [("ccd_temp", self.temp_var), ("iso", self.iso_var),
    ...
    def create_master_dark(self):
        ...
        lib_name = re.sub(r'[^A-Za-z0-9_-]', '_', self.selected_library.get())
        master_path = self.target_dir.get() + "/" + lib_name
        ...
        master_path = self.target_dir.get() + "/" + lib_name + "/" +  self.generate_master_name()
        siril.cmd("cd", f"{tmpdir}")
